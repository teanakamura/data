...
from apply_bpe_with_separated_annotation import BPE, read_vocabulary
...
from apply_bpe_with_separated_annotation import create_parser as create_apply_bpe_parser
...
learn-joint-bpe-and-vocab: executes recommended workflow for joint BPE.""")
    ...
    if args.command == 'learn-bpe':
        ...
        if args.annotation_input:
            args.annotation_input = codecs.open(args.annotation_input.name, encoding='utf-8')
        if args.annotation_output.name != '<stdout>':
            args.annotation_output = codecs.open(args.annotation_output.name, 'w', encoding='utf-8')
        ...
        for line, annt in zip(args.input, args.annotation_input):
            out, annt_out = bpe.process_line(line, annt, args.dropout)
            args.output.write(out)
            args.annotation_output.write(annt_out)
