...
class BPE(object):
    ...
    def process_line(self, line, annt, dropout=0):
        ...
        leading_whitespace = len(annt)-len(annt.lstrip('\r\n '))
        if leading_whitespace:
            annt_out += annt[:leading_whitespace]

        new_line, new_annt = self.segment(line, annt, dropout)
        out += new_line
        annt_out += new_annt
        ...
        trailing_whitespace = len(annt)-len(annt.rstrip('\r\n '))
        if trailing_whitespace and trailing_whitespace != len(annt):
            annt_out += annt[-trailing_whitespace:]

        return out, annt_out

    def segment(self, sentence, annt, dropout=0):
        """segment single sentence (whitespace-tokenized string) with BPE encoding"""
        segments, annts = self.segment_tokens(sentence.strip('\r\n ').split(' '), annt.strip('\r\n ').split(' '), dropout)
        return ' '.join(segments), ' '.join(annts)

    def segment_tokens(self, tokens, annts, dropout=0):
        """segment a sequence of tokens with BPE encoding"""
        assert len(tokens) == len(annts), 'Annotation length does not match.'
        output = []
        annt_output = []
        for word, binary in zip(tokens, annts):
            ...
            annt_output += [binary] * len(new_word)
            for item in new_word[:-1]:
                output.append(item + self.separator)
            output.append(new_word[-1])

        return output, annt_output
...
def create_parser(subparsers=None):
    ...
    parser.add_argument(
        '--annotation-input', '--ai', '-a', type=argparse.FileType('r'), metavar='PATH',
        required=True,
        help="File with words annotation.")
    parser.add_argument(
        '--annotation-output', '--ao', '-A', type=argparse.FileType('w'), default=sys.stdout,
        metavar='PATH',
        help="Annotation output file (default: standard output)")
